...
function Window:clone() return Window({ file = self.file }) end

---@return boolean
function Window:is_valid() return self.id and api.nvim_win_is_valid(self.id) end
...
function Window:focus()
  if self:is_valid() then api.nvim_set_current_win(self.id) end
end

function Window:is_focused() return self:is_valid() and api.nvim_get_current_win() == self.id end

function Window:post_open() self:apply_custom_folds() end
...
function Window:open_fallback()
  ...
  if self:show_winbar_info() then vim.wo[self.id].winbar = self.file.winbar end
...
Window.open_file = async.void(function(self)
  ...
  if self.file.rev.type == RevType.LOCAL then self:_save_winopts() end
...
function Window:detach_file()
  if self.file and self.file:is_valid() then self.file:detach_buffer() end
end
...
function Window:_restore_winopts()
  ...
  then
    utils.no_win_event_call(function()
      ...
      if HAS_NVIM_0_8 then vim.wo[winid].winbar = nil end
...
function Window:apply_file_winopts()
  assert(self.file)
  if self.file.winopts then utils.set_local(self.id, self.file.winopts) end
end

function Window:apply_null_winopts()
  if File.NULL_FILE.winopts then utils.set_local(self.id, File.NULL_FILE.winopts) end

  local file_winhl = utils.tbl_access(self, "file.winopts.winhl")
  if file_winhl then utils.set_local(self.id, { winhl = file_winhl }) end
end
...
function Window:use_winopts(opts)
  ...
  api.nvim_win_call(self.id, function()
    for option, v in pairs(opts) do
      if opt_store[option] == nil then opt_store[option] = vim.o[option] end
...
function Window:apply_custom_folds()
  if self.file.custom_folds and not self:is_nulled() and vim.wo[self.id].foldmethod == "manual" then
    api.nvim_win_call(self.id, function()
...
function Window:set_id(id) self.id = id end

function Window:set_file(file) self.file = file end
