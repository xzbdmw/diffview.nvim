...
function HgRev:init(rev_type, revision, track_head)
  ...
  if t == "string" then assert(revision ~= "", "'revision' cannot be an empty string!") end
...
function HgRev.new_null_tree() return HgRev(RevType.COMMIT, HgRev.NULL_TREE_SHA) end
