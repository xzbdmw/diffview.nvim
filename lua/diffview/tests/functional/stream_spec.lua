...
describe("diffview.stream", function()
  ...
  describe("Stream", function()
    it("can collect a simple array", function() eq(arr1, Stream(arr1):collect()) end)
    ...
    it(
      "can slice",
      function() eq(vim.list_slice(arr1, 2, 4), Stream(arr1):slice(2, 4):collect()) end
    )

    it("can map", function()
      local function f(item) return item.name:upper():rep(5) end

      eq(vim.tbl_map(f, arr1), Stream(arr1):map(f):collect())
    end)

    it("can filter", function()
      local function f(item) return item.i % 2 ~= 0 end
    ...
    it("can reduce without an init value", function()
      eq(
        "abcde",
        Stream({ "a", "b", "c", "d", "e" }):reduce(function(acc, cur) return acc .. cur end)
      )
    end)

    it("can reduce with an init value", function()
      eq("init abcde", Stream(arr1):reduce(function(acc, cur) return acc .. cur.name end, "init "))
    end)
    ...
    it("can run a pipeline of transforms", function()
      eq(
        "AAAAACCCCCEEEEE",
        Stream(arr1)
          :filter(function(item) return item.i % 2 ~= 0 end)
          :map(function(item) return item.name:upper():rep(5) end)
          :reduce(function(acc, cur) return acc .. cur end, "")
      )
    end)
  end)
  ...
  describe("AsyncListStream", function()
    ...
    it(
      ...
      async_test(function()
        ...
        stream = AsyncListStream({
          on_close = function() stream:push(unpack(final_arr)) end,
        })
        mock_worker(stream)
    ...
    it(
      ...
      async_test(function()
        local ret = {}
        local stream = AsyncListStream({
          on_close = function() table.insert(ret, 1) end,
          on_post_close = function() table.insert(ret, 2) end,
        })
